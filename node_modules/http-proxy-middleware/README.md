...
Proxy `/api` requests to `https://www.example.org`
...
app.use('/api', createProxyMiddleware({ target: 'https://www.example.org', changeOrigin: true }));
app.listen(3000);
...
:bulb: **Tip:** Set the option `changeOrigin` to `true` for [name-based virtual hosted sites](https://en.wikipedia.org/wiki/Virtual_hosting#Name-based).
...
const apiProxy = createProxyMiddleware('/api', { target: 'https://www.example.org' });
...
const apiProxy = createProxyMiddleware('https://www.example.org/api');
```
...
const options = {
  target: 'https://www.example.org', // target host
...
Providing an alternative way to decide which requests should be proxied; In case you are not able to use the server's [`path` parameter](https://expressjs.com/en/4x/api.html#app.use) to mount the proxy or when you need more flexibility.
...
- **option.router**: object/function, re-target `option.target` for specific requests.
  ...
  router: {
      'integration.localhost:3000' : 'https://localhost:8001',  // host only
      'staging.localhost:3000'     : 'https://localhost:8002',  // host only
      'localhost:3000/api'         : 'https://localhost:8003',  // host + path
      '/rest'                      : 'https://localhost:8004'   // path only
  }
...
- **option.agent**: object to be passed to http(s).request (see Node's [https agent](https://nodejs.org/api/https.html#https_class_https_agent) and [http agent](https://nodejs.org/api/http.html#http_class_http_agent) objects)
...
```javascript
createProxyMiddleware('https://www.example.org:8000/api');
// createProxyMiddleware('/api', {target: 'https://www.example.org:8000'});

createProxyMiddleware('https://www.example.org:8000/api/books/*/**.json');
// createProxyMiddleware('/api/books/*/**.json', {target: 'https://www.example.org:8000'});

createProxyMiddleware('https://www.example.org:8000/api', { changeOrigin: true });
// createProxyMiddleware('/api', {target: 'https://www.example.org:8000', changeOrigin: true});
```
...
- express: https://expressjs.com/en/4x/api.html#app.use
...
```javascript
// verbose api
createProxyMiddleware('/', { target: 'https://echo.websocket.org', ws: true });

// shorthand
createProxyMiddleware('https://echo.websocket.org', { ws: true });
